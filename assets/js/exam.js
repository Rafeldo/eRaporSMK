...
jQuery(function() {
  if (jQuery('#exam-ui').length) {
    ...
    success: function(response) {
	  ...
	  if(data.userexam_status == 'completed'){
		  finishExam();
	  }
	  waktu = diff;
      examId = data.id;
	...
	jQuery('#view-fullscreen').click(function(e){
  if (jQuery('#exam-time-left').length && EXAM_TIME_LEFT) {
    updateExamTimer();
	setInterval(function(){recordTime()},1800000);
  }
  });
});
...
function recordTime() {
	jQuery.ajax({
				...
				url: '../save_time',
				async: false,
				data: {id: examId,waktu:jQuery('#exam-time-left').val(),userexam_id:jQuery('#userexam_id').val()},
				success: function(response) {
...
//	console.log(terjawab);
    ...
    jQuery.ajax({
      ...
      async: false, 
	  //data: {id: examId,waktu:jQuery('#exam-time-left').val(),userexam_id:jQuery('#userexam_id').val()},
      data: {id: examId, q: jQuery('#question-id').val(), a: answerId,userexam_id:jQuery('#userexam_id').val(),waktu:jQuery('#exam-time-left').val()},
