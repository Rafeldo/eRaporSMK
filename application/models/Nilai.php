...
class Nilai extends ActiveRecord\Model{
	...
	public function after_create_log(){
		...
		set_log('Tambah nilai siswa','ID:'.$this->id,$loggeduser->username);
	}
	public function after_update_log(){
		$CI = & get_instance();
		$CI->load->library('ion_auth');
		$loggeduser = $CI->ion_auth->user()->row();
		set_log('Update nilai siswa','ID:'.$this->id,$loggeduser->username);
	}
	public function before_destroy(){
		$CI = & get_instance();
		$CI->load->library('ion_auth');
		$loggeduser = $CI->ion_auth->user()->row();
		set_log('Hapus nilai siswa','ID:'.$this->id,$loggeduser->username);
	}
}