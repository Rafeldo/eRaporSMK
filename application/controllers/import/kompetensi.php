...
class Kompetensi extends Backend_Controller {
	...
	public function index(){
		...
		if($highestColumn == 'B') { // Import data siswa
		...
		foreach($masukkan as $k=>$v){
			$a = Datakurikulum::all(array('conditions' => array('kurikulum_id = ? AND nama_kurikulum = ?', $v['kurikulum_id'], $v['nama_kurikulum'])));
			$sum+=count($a);
			if(!$a){
				$inser_data = Datakurikulum::create($v);
				if(!$inser_data){
